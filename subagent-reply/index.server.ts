/**
 * subagent-reply — scoped child->parent reply door for Paseo.
 *
...
export default function contribute(server: PluginServerContext): PluginCleanup {
  ...
  listenReplyServer({
    ...
    .then((handle) => {
      replyServer = handle;
      console.log(
        `[subagent-reply] listening on 127.0.0.1:${handle.port} (allowFull=${allowFull ? "1" : "0"}); ` +
          `spawn children with --env PASEO_PARENT_AGENT_ID[=id][,PASEO_CHILD_MODE=knob] to get the scoped door`,
      );
    })
    .catch((err: unknown) => {
      console.error(`[subagent-reply] FAILED to listen: ${err instanceof Error ? err.message : String(err)}`);
    });
